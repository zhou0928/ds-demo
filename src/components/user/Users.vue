...
export default {
    ...
    return {
      ...
      addFormRules: {
        username: [
          ...
          {min: 3, max: 10, message: '用户名应在3~10个字符', trigger: "blur"}
        ],
        password: [
          {required: true, message: '请输入密码', trigger: 'blur'},
          {min: 6, max: 15, message: '密码长度应为6~15位之间', trigger: 'blur'},
  ...
  methods: {
    async getUserList() {
      let {data: res} = await axios.get('users', {params: this.queryInfo})
      // console.log(res)
    ...
    addUser() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        if (!valid) return
        // 可以发起添加用户的网络请求
        let {data: res} = await axios.post('users', this.addForm)
        console.log(res)
